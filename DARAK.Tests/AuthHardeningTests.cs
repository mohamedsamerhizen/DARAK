...
public sealed class AuthHardeningTests
{
    ...
    [Theory]
    [InlineData(typeof(LoginRequest), nameof(LoginRequest.Email), 256)]
    [InlineData(typeof(LoginRequest), nameof(LoginRequest.Password), 128)]
    [InlineData(typeof(RegisterRequest), nameof(RegisterRequest.Email), 256)]
    [InlineData(typeof(RegisterRequest), nameof(RegisterRequest.Password), 128)]
    [InlineData(typeof(RefreshTokenRequest), nameof(RefreshTokenRequest.RefreshToken), 512)]
    [InlineData(typeof(LogoutRequest), nameof(LogoutRequest.RefreshToken), 512)]
    public void AuthDtos_DefineMaximumInputLengths(Type requestType, string propertyName, int expectedMaximumLength)
    {
        var attribute = requestType.GetProperty(propertyName)!
            .GetCustomAttribute<System.ComponentModel.DataAnnotations.MaxLengthAttribute>();

        attribute.Should().NotBeNull();
        attribute!.Length.Should().Be(expectedMaximumLength);
    }
